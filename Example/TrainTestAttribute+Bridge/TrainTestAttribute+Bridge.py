...
while counter < 5:
    AllNodeFeatureNum = []
    AllNodeFeatureNumName = 'AllNodeAttributeBridgeNum' + str(counter) + '.csv'
...
StorFile(MyNew, '5折的表-attribute+bridge+xg+sdne.csv')
...
# 保存图片
plt.savefig('5fold-attribute+bridge+xg+sdne.svg')
plt.savefig('5fold-attribute+bridge+xg+sdne.tif')
plt.show()