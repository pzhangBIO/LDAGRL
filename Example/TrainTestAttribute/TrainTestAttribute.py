...
while counter <5:
    ...
    AllNodeFeatureNumP = np.array(AllNodeFeatureNum)#[:, 1:]
    ...
    MyEnlarge(0.15, 0.55, 0.20, 0.20, 0.6, 0.0, 2, mean_fpr, tprs[i], 1.5, colorlist[i])
    # 保存5个评价指标和auc
...
StorFile(MyNew, '5折的表-attribute+sdne.csv')
...
plt.plot(mean_fpr, mean_tpr, color='black',
         ...
         lw=2, alpha=1)
MyEnlarge(0.15, 0.55, 0.20, 0.20, 0.6, 0.0, 2, mean_fpr, mean_tpr, 1.5, colorlist[10])
...
# 保存图片
plt.savefig('5fold-attribute+sdne.svg')
plt.savefig('5fold-attribute+sdne.tif')
plt.show()