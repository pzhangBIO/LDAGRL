...
while counter < 5:
    AllNodeFeatureNum = []
    AllNodeFeatureNumName = 'AllNodeBridgeNum' + str(counter) + '.csv'
    # print('AllNodeFeatureNumName', AllNodeFeatureNumName)
    AllNodeFeatureNum1 = []
    AllNodeFeatureNumName1 = 'AllNodeAttribute.csv'
    ReadMyCsv3(AllNodeFeatureNum, AllNodeFeatureNumName)
    ReadMyCsv(AllNodeFeatureNum1, AllNodeFeatureNumName1)
    AllNodeFeatureNumP = np.array(AllNodeFeatureNum1)  # [:, 1:]
    ...
    # testSample = []
    # ReadMyCsv3(testSample, "testSample/prostateTestSampleV.csv")
    # testSampleFeature = []
    # counterT = 0
    # while counterT < len(testSample):
    #     featurePair = []
    #     featurePair.extend(AllNodeFeatureNum[int(testSample[counterT][0])])
    #     featurePair.extend(AllNodeFeatureNum[int(testSample[counterT][1])])
    ...
    # y_score0_0 = model1.predict(np.array(testSampleFeature))
    # y_score1_1 = model1.predict_proba(np.array(testSampleFeature))
    # print(y_score0_0)
    # StorFile(y_score1_1, "testResult/prostateResultV.csv")
...
StorFile(MyNew, '5折的表-bridge+sdne.csv')
...
# 保存图片
plt.savefig('5fold-bridge+sdne.svg')
plt.savefig('5fold-bridge+sdne.tif')
plt.show()
